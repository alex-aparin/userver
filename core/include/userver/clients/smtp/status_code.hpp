...
template <>
struct fmt::formatter<USERVER_NAMESPACE::smtp::StatusCode> {
    constexpr static auto parse(format_parse_context& ctx) { return ctx.begin(); }

    template <typename FormatContext>
    auto format(USERVER_NAMESPACE::smtp::StatusCode status, FormatContext& ctx) const {
        return fmt::format_to(
            ctx.out(),
            "{} {}",
            static_cast<int>(status),
            USERVER_NAMESPACE::smtp::StatusCodeString(status)
        );
    }
};
